...
void Serial()
{
	...
	string ComPortName = "\\\\.\\"+ Port;  // Name of the Serial port(May Change) to be opened,
	...
	std::wstring stemp = std::wstring(ComPortName.begin(), ComPortName.end());
	...
	hComm = CreateFile(a,                  // Name of the Port to be Opened
	...
	else //If  WaitCommEvent()==True Read the RXed data using ReadFile();
	{
		...
		do
		{
			...
			i++;
			//printf("%c", SerialBuffer[i - 1]);
			File << SerialBuffer[i - 1];
			if (SerialBuffer[i - 1] == '\n') {
				i = 0;
...
void setImuPath(string ImuPath) {
	IMUPath = ImuPath;
}

void setPort(string ImuPort) {
	Port = ImuPort;
}