...
StereoCalib(const vector<string>& imagelist, Size boardSize, float squareSize, bool displayCorners = true, bool useCalibrated = true, bool showRectified = true)
{
		...
		for (k = 0; k < 2; k++)
		{
			...
			for (int scale = 1; scale <= maxScale; scale++)
			{
				...
				if (scale == 1)
					...
					cv::resize(img, timg, Size(1920, 1080), INTER_LINEAR_EXACT);
	...
	/*GpuR1.upload(R1); GpuR2.upload(R2); GpuP1.upload(P1); GpuP2.upload(P2); GpuQ.upload(Q);*/	fs.open("extrinsics.yml", FileStorage::WRITE);
	if (fs.isOpened())
	{
...
int RunStereoCalibrate(string StereoCalibrationXml,int BoardSizeWidth=9,int BoardSizeHeight=6, float SquareSize=25, bool ShowRectified = false)
{
	Size BoardSize;
	...
	bool ok = readStringList(StereoCalibrationXml, ImageList);
	if (!ok || ImageList.empty())
	{
		cout << "can not open " << StereoCalibrationXml << " or the string list is empty" << endl;
		return print_help();
	}
